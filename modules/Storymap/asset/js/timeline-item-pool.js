/**
 * Manage events for storymap blocks.
 */
(function ($) {
    /**
    ...
    $(document).ready(function () {
        ...
        $('#blocks').find('.block[data-block-layout="storymap"]').each(function() {
            initItemPool(this);
        });

        // Bind events on the advanced search form for item pool of added blocks.
        $('#blocks').on('o:block-added', '.block[data-block-layout="storymap"]', function(e) {
            $(this).filter('.block[data-block-layout="storymap"]').each(function() {
                initItemPool(this);
            });
        });
